...
void Dac_ON(dac_channel_x channel)
{
	...
	//RTCIO.pad_dac[channel].dac_xpd_force = 1; // activaci�n forzada de la alimentaci�n (power-down) del DAC.
	//RTCIO.pad_dac[channel].xpd_dac = 1;			// activaci�n de la alimentaci�n (power-down) del DAC.
...
void Dac_OFF(dac_channel_x channel)
{
    ...
    //RTCIO.pad_dac[channel].dac_xpd_force = 0;  // activaci�n forzada de la alimentaci�n (power-down) del DAC.
    //RTCIO.pad_dac[channel].xpd_dac = 0;	// activaci�n de la alimentaci�n (power-down) del DAC.
}
/****************************************************************************
	...
	//Instanciamos la estructura con los valores del usuario
	 ...
	 parametros.channel = channel; //Valor del canal
	   ...
	   //Se crea una tarea que mande a llamarejecutar el generador de onda con los valores que se guardadron en la esctructura
	    ...
	    //Este es el tama�o del stack (pila) 3KB (1kb = 1024) * (3) = 3kb
}
...
void Config_CW(void *parametros){
	// Convierte el puntero a la estructura de par�metros
	    ParametrosConfigCW *misParametros = (ParametrosConfigCW *)parametros;

	    // Accede a los par�metros de la estructura
	 ...
	 // Se habilita el generador de ONDA (este se usa en ambos canales)por lo que se hace solo 1 vez sin importar e canal a usar
	    ...
	    //Ya por ultimo con un while se manda a la salida del DAC (PIN) el valor de la frecuencia que se establecio
	            while(1){
	            	...
	            	FREC_DAC(frecuencia)
	                   vTaskDelay(2000/10);	//funci�n de FreeRTOS,hacer que una tarea se suspenda (se detenga) durante un per�odo de tiempo espec�fico antes de volver a ejecutarse
